...
func TestMemoryProductRepository_Get (t *testing.T) {
	...
	testCases := []testCase{
		{
			name: "Get product by id", id: existingProd.GetID(), expectedErr: nil,
		}, {
			name: "Get non-existing product by id", id: uuid.New(), expectedErr: product.ErrProductNotFound,
		},
	}
...
func TestMemoryProductRepository_Delete (t *testing.T) {
	...
	if len(repo.products) != 0 {
		t.Errorf("Expected 0 product, got %d", len(repo.products))
	}
}